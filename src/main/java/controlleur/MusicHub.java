...
public class MusicHub {
	...
	public List<String> GetAllBySearch(String sc){
		...
		for(Album al :this.albums){
			if(al.getTitle().toLowerCase().contains(sc.toLowerCase()))
				ls.add(al.getTitle());
		}

		for(PlayList pl :this.playlists){
			if(pl.getTitle().toLowerCase().contains(sc.toLowerCase()))
				ls.add(pl.getTitle());
		}
...
*/
	/*
	 * Permet de recuperer les elements audio selon une chaine de caractere que tape l'utilisateur 
	 * 
	 * @param chaine de caractere qui peut être present dans un audio elements
	 * 
	 * 
	 * */
	public List<AudioElement> GetAllBySearchAE(String sc){
		...
		for(AudioElement ae :this.elements){
			if(ae.getTitle().toLowerCase().contains(sc.toLowerCase()))
				ls.add(ae);
		}
	...
	public String GetElementContent(String title){
		...
		for(AudioElement ae : this.elements){
			if(ae.getTitle().toLowerCase().equals(title.toLowerCase())){
